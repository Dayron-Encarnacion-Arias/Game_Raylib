...
void Character::Draw(Vector2 body, float scale, int pose, float time, bool locked) const
{
    ...
    const float arm = pose % 3 == 0 ? -0.7f : pose % 3 == 1 ? 0.45f : std::sin(time * 4.0f);
    ...
    DrawCircleV({head.x+5*scale,head.y-scale},2*scale,locked?DARKGRAY:WHITE);
    if (!locked) DrawAccessory(body, head, scale);
}
...
{
    if (accessory == 1) DrawRectangleRounded({head.x-13*z,head.y-4*z,26*z,7*z},0.5f,4,lightRed);
    if (accessory == 2) { DrawRectangleV({head.x-13*z,head.y-15*z},{23*z,6*z},lightRed); DrawRectangleV({head.x+5*z,head.y-10*z},{14*z,4*z},lightRed); }
    if (accessory == 3) { DrawCircleLines(head.x,head.y,17*z,lightRed); DrawRectangleV({head.x-19*z,head.y-4*z},{5*z,12*z},lightRed); DrawRectangleV({head.x+14*z,head.y-4*z},{5*z,12*z},lightRed); }
    if (accessory == 4) { DrawLineEx({head.x,head.y-13*z},{head.x,head.y-27*z},3*z,lightRed); DrawCircleV({head.x,head.y-29*z},4*z,lightRed); }
    if (accessory == 5) for (int i=-1;i<=1;i++) DrawTriangle({head.x+(i*9-5)*z,head.y-12*z},{head.x+i*9*z,head.y-(i?24:29)*z},{head.x+(i*9+5)*z,head.y-12*z},GOLD);
    if (accessory == 7) { DrawLineEx({body.x+16*z,body.y+11*z},{body.x+29*z,body.y-5*z},4*z,LIGHTGRAY); DrawCircleLines(body.x+31*z,body.y-8*z,6*z,LIGHTGRAY); }
    if (accessory == 8) DrawEllipseLines(head.x,head.y-22*z,18*z,5*z,GOLD);
    if (accessory == 9) { DrawTriangle({head.x-12*z,head.y-10*z},{head.x-20*z,head.y-26*z},{head.x-4*z,head.y-14*z},lightRed); DrawTriangle({head.x+12*z,head.y-10*z},{head.x+20*z,head.y-26*z},{head.x+4*z,head.y-14*z},lightRed); }
    if (accessory == 10) { DrawRectangleRounded({head.x-12*z,head.y-8*z,24*z,17*z},0.35f,5,{35,25,29,255}); DrawLineEx({head.x-7*z,head.y},{head.x+7*z,head.y},2*z,GOLD); }
}
