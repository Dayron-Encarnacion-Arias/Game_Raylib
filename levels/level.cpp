...
void Level::DrawMap(float time, int width, int height) const
{
    ...
    if (id == 0) for (int i=0;i<12;i++) DrawRectangle(i*120-static_cast<int>(std::fmod(time*24,120.0f)),80+(i%3)*150,82,12,Fade(primary,0.20f));
    if (id == 1) for (int i=0;i<9;i++) DrawCircle(80+i*155,650-static_cast<int>(std::fmod(time*(18+i),380.0f)),20+i%3*8,Fade(primary,0.15f));
    if (id == 2) for (int i=0;i<18;i++) { int h=80+(i*47)%220; DrawRectangle(i*76,height-h,55,h,{15,10,25,255}); DrawRectangle(i*76+8,height-h+15,5,14,Fade(primary,0.5f)); }
    if (id == 3) for (int radius=90;radius<470;radius+=70) DrawCircleLines(width/2,height/2,radius+std::sin(time*2)*7,Fade(primary,0.18f));
    if (id == 4) for (int i=0;i<8;i++) { DrawRectangle(i*180,100,90,620,{17,10,20,255}); DrawCircle(i*180+45,100,45,Fade(primary,0.16f)); }
    if (id == 5) for (int i=0;i<7;i++) { float x=std::fmod(i*233.0f+time*90,1400.0f)-60; DrawLineEx({x,0},{x-220,720},2,Fade(primary,0.22f)); }
    if (id == 6) for (int i=0;i<9;i++) { Vector2 c={70.0f+i*150,110.0f+(i%3)*240}; DrawCircleLines(c.x,c.y,50,Fade(primary,0.18f)); for(int k=0;k<8;k++) DrawRectanglePro({c.x,c.y,70,8},{0,4},k*45.0f+time*10,Fade(primary,0.12f)); }
    if (id == 7) for (int i=0;i<80;i++) DrawCircle((i*97)%width,(i*53+static_cast<int>(time*(i%3+1)*3))%height,i%3+1,Fade(WHITE,0.25f));
    if (id == 8) for (int i=0;i<14;i++) DrawTriangle({i*100.0f,720},{i*100.0f+50,480.0f+std::sin(time+i)*35},{i*100.0f+100,720},Fade(primary,0.11f));
    if (id == 9) for (int i=0;i<12;i++) { float a=i*PI/6+time*0.05f; DrawLineEx({width/2.0f,height/2.0f},{width/2.0f+std::cos(a)*900,height/2.0f+std::sin(a)*900},22,Fade(primary,0.08f)); }
